...
$user_list = 'Legislators';
$file = "Legislators Database.xls";
		...
		while($x <= $sheet['numRows']) {
			...
			while($y <= $sheet['numCols']) {				
				...
				else if($y==3)	//Email Field
				{
					...
					$wp_user_values[] = trim($cell); //user login 
					$wp_user_values[] = trim($cell); //user email
					
					$cell_url = isset($sheet['cells'][$x][18]) ? $sheet['cells'][$x][18] : '';
					$wp_user_values[] = addslashes(trim($cell_url)); //user url
					...
					$cell_img = isset($sheet['cells'][$x][17]) ? $sheet['cells'][$x][17] : '';
					$wp_usermeta_values[] = " VALUES('cupp_meta','".addslashes(trim($cell_img))."', '%s')";
				...
				else if($y==16)	//Representing Field
				{
					//Another Fields For User Table
					$cell = isset($sheet['cells'][$x][$y]) ? $sheet['cells'][$x][$y] : '';
					$cell = strtoupper(str_replace(", ",",",$cell));
					
					$wp_cimy_uef_data_values[]  = " VALUES('15','".addslashes(trim($cell))."', '%s')"; 
				}
				else if($y==19)	//Middle Field
				{
					//Another Fields For User Table
					$cell = isset($sheet['cells'][$x][$y]) ? $sheet['cells'][$x][$y] : '';
					$wp_cimy_uef_data_values[]  = " VALUES('10','".addslashes(trim($cell))."', '%s')"; 
				}
				else if($y==20)	//Suffix Field
				{
					//Another Fields For User Table
					$cell = isset($sheet['cells'][$x][$y]) ? $sheet['cells'][$x][$y] : '';
					$wp_cimy_uef_data_values[]  = " VALUES('11','".addslashes(trim($cell))."', '%s')"; 
				}
