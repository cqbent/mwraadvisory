     ...
     query_posts($argue_non_sticky_post);
	  ?>
		...
		<h2 class="screen-reader-text">Posts navigation</h2>
             <div class="nav-links"><div class="nav-next"><a href="<?php echo $url;?>">Newer posts</a></div></div> 
	  ...
	  <nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text">Posts navigation</h2>
         <div class="nav-links"><div class="nav-previous"><a href="<?php echo $url; ?>?page_no=show_all">All Recent Posts</a></div></div> 
		...
		case 'legislators':
			...
			if(count($result)>0){
				foreach($result as $row){
					...
					if($row->user_url!=""){
						?>
	                    <p><?php echo $row->job_title?> <a href="<?php echo $row->user_url;?>" target="_blank"><?php echo $row->name?></a></p>
			            <?php
					}					
					else
					{
...
function show_executive_committee_table( $atts ) {	
	...
	?>
    <style>
	.executive-committee-table table{ width:auto; margin:0px; padding:0px; }
	.executive-committee-table table tr td{ border:none !important; padding:0px !important; margin:0px; }
	.executive-committee-table table tr td img{ padding:2px !important; }
	.fancybox_content{ font-weight:normal; min-width:600px; padding-bottom:10px; }
	.fancybox_content img{ float:left;margin:0px 20px 20px 0px; }
	.fancybox_content h6{ clear:none;padding-bottom:0px; } 
	.fancybox_content span.position{ text-transform:uppercase;color:#939597; }	
	</style>
						...
						<td valign="middle"><table>
                                <tr>
                                    <td valign="middle">
                                        ...
                                        <?php }else{ ?>
											<img src="<?php echo get_stylesheet_directory_uri();?>/images/noimage.png" width="50" />
										<?php }?>    
                                        </a>
                                    </td> 
                                    <td valign="middle">&nbsp; <a class="fancybox-inline" href="#officer<?php echo $r;?>"><?php echo $row->name;?></a>
                                    
                                    	<div style="display:none" class="fancybox-hidden">                                        	
                                              <div id="officer<?php echo $r;?>" style="overflow-x: auto;overflow-y: auto;">
                                                <div class="fancybox_content"> 
                                                    
													<?php if($profile_photo!=''){	?>
                                                        <img class="fancyimg" src="<?php echo $profile_photo;?>"/>
                                                    <?php }else{ ?>
                                                        <img class="fancyimg" src="<?php echo get_stylesheet_directory_uri();?>/images/noimage.png"/>
                                                    <?php }?>  
					...
					<tr>
						<td valign="middle"><table>
                                <tr>
                                    ...
                                    <td valign="middle">&nbsp; <a class="fancybox-inline" href="#member_at_large<?php echo $r;?>"><?php echo $row->name;?></a>
                                    
                                    	<div style="display:none" class="fancybox-hidden">                                        	
                                              <div id="member_at_large<?php echo $r;?>" style="overflow-x: auto;overflow-y: auto;">
                                                <div class="fancybox_content"> 
...
function my_show_extra_profile_fields( $user ) { //echo '<pre>'; print_r($user);
	global $wpdb;	
	...
	<table class="form-table">
		<tr>
			...
			<td>
				...
				$unsub_text = ''; 
				$select = '';
				if(is_array($_POST['wysija']['user_list']['list_id']) && isset($_POST['wysija']['user_list']['list_id']) && in_array($row->list_id,$_POST['wysija']['user_list']['list_id'])){ $select = ' checked'; }
				...
				{						
					...
					if(count($result2)>0){ 						
						...
						?>
						<p class="labelcheck"><input name="wysija[user_list][list_id][]" value="<?php echo $row->list_id;?>" id="list<?php echo $row->list_id;?>" class="" type="checkbox" <?php echo $select;?> ><label for="list<?php echo $row->list_id;?>"><?php echo $row->name;?></label> <?php echo $unsub_text;?></p>
						<?php
					}
				}
				?>
...
function form29_table_fun($form){ //echo 'fff'; echo '<pre>'; print_r($form); exit;
	return $form;
}
