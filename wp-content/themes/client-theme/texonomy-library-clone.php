...
/* Template Name: taxonomy-library-clone */
...
<div class="libraries-search-section">
    ...
    ?>
     ...
     <p><a href="#categories_list" id="categories" style="font-size:14px; line-height:26px; color:white; padding: 6px; background-color: red;">See All Categories</a></p>
   <!-- 		<p >
					...
					//	echo "<pre>";print_r($posts);
						foreach($posts as $latest_post){
						    ...
						    setup_postdata($latest_post);
								...
								$library_thumbnail_url = wp_get_attachment_image_src( get_post_thumbnail_id($latest_post->ID), 'medium' );
						...
						<?php if($library_type == 'video'){?>			
        					...
        					<h4 style="font-size: 13px;left;width: 220px;word-break: initial; padding-top:17px;"><?php echo $latest_post->post_title; ?></h4>
            		     	<?php } else{ ?>
                				<a href="#pdf_document-<?php echo $library->ID ?>" class="fancybox-inline"><img src="<?php echo $library_thumbnail; ?>" style="height:250px;width:200px;"/></a>
                				<div style="display:none" class="fancybox-hidden">
                       ...
                       <?php 
						  $i++;   
						    ...
						    if($i==4)
            				    {?>
            				        <li style='float:right; margin-top:75px;'><a onclick="window.location.href='<?php echo get_category_link($library_category->term_id ).'#category_content'?>'; return false;" href='#' style='color:white; width:120px; background-color:red; padding:6px; font-size:14px;'>See All</a></li>
            				 <?php   }
...
$("#categories").click(function() {
 $('html, body').animate({
        scrollTop: $("#categories_list").offset().top
    }, 1000);
});
</script>
