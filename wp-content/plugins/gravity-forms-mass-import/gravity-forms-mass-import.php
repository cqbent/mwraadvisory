 ...
 * 
*/
/*Enqueue the javascript that calls the ajax when you select a form and the headers table shows up*/

function impform_scripts_plug() {
	if(is_admin()){
		wp_register_script( 'backcallsc', plugins_url( 'js/backtocall.js', __FILE__ ), array('jquery'));
		wp_enqueue_script('backcallsc');
...
add_action('init', 'impform_scripts_plug');
/*This is the ajax callback.  dataVal is passed with the form number and get_field_list_massimport is called*/
add_action('wp_ajax_give_backstuff', 'aad_callback');

add_action('wp_ajax_nopriv_give_backstuff', 'aad_callback');
...
/*Get all the fields relative to the selected form.  Filter out headers that are not stored in the database or that I have not supported because I was too lazy.  Return table html that displays a list of valid headers and how to info.*/
...
function get_field_list_massimport($num){
	...
	for($i=0;$i<sizeof($ararr);$i++){
		$checkarey = array("section","page","fileupload","captcha","post_title","post_content","post_excerpt","post_tags","post_category","post_image","post_custom","product","quantity","option","shipping","total");/*"html",*/
		
		if(in_array($ararr[$i]['type'], $checkarey)){}
		elseif($ararr[$i]['id'] == null){}
		else{
			if($j%2 == 0){
...
function get_meta_all_fields($id){
	...
	foreach($iterator as $key=>$value) {
        ...
        */
        foreach($fldobj as $keys => $values){
            $fledarr[$keys] = array();
            $fledarr[$keys]['label'] = $values->label;
            $fledarr[$keys]['type'] = $values->type;
            $fledarr[$keys]['id'] = $values->id;
            $fledarr[$keys]['value'] = '';
            
            if($values->type == 'list'){
             //$fledarr[$keys]['choices'] = $values->choices;  
             $list_column = array();
             foreach($values->choices as $choice_arr){
                 $ckey = $choice_arr['value'];
                 $cvalue = $choice_arr['text'];
                 $list_column[$ckey]= $cvalue;
             }
              $fledarr[$keys]['choices'] = $list_column;    
            }
            
             if($values->type == 'checkbox'){
                
                  $list_inputs = array();
             foreach($values->inputs as $input_arr){
                 $ckey = $input_arr['id'];
                 $cvalue = $input_arr['label'];
                 $list_inputs[$ckey]= $cvalue;
             }
              $fledarr[$keys]['inputs'] = $list_inputs; 
...
require('parsecsv.lib.php');
		if(isset($_POST['formnum'])) {
			...
			$csv = new parseCSV($_FILES['csv_file']['']);
