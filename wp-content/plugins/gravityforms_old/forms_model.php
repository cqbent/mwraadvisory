...
class GFFormsModel {
	...
	public static function save_lead( $form, &$lead ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			$field_container = GFFormDisplay::get_field_content( $field, '', false, $form == null ? 0 : $form['id'], $form);
			
			if($field->type=='html' && stristr($field_container,'<input')){
				//echo $field->id."-".$field->type."-".$field->displayOnly;
				//echo '<br />';			
			}
			else
			{
	...
	public static function choice_value_match( $field, $choice, $value ) {
		...
		$choice_value = strip_tags($choice_value);	
