	...
	public function get_field_input( $form, $value = '', $entry = null, $customize=0 ) {
		...
		if ( is_array( $payment_methods ) ) {
			foreach ( $payment_methods as $payment_method ) {
				...
				if($customize==1)
				{
					$payment_options .= "<div class='gform_payment_option gform_payment_{$payment_method['key']}'><input type='radio' name='gform_payment_method' value='{$payment_method['key']}' id='gform_payment_method_{$payment_method['key']}' onclick='gformToggleCreditCard();' onkeypress='gformToggleCreditCard();' {$checked} disabled /> {$payment_method['label']}</div>";
				}
				else
				{
