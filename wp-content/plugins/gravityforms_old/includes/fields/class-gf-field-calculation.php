...
class GF_Field_Calculation extends GF_Field {
	...
	public function get_field_input( $form, $value = '', $entry = null, $customize=0 ) {
		...
		if ( $is_entry_detail || $is_form_editor  ) {
			...
			if($customize==1)
			{
				$quantity_field = " <span class='ginput_quantity_label' {$style}>{$product_quantity_sub_label}</span> ".$quantity;		
			}
			else
			{
			...
			if($customize==1)
			{
				$quantity_field .= " <span class='ginput_quantity_label'>" . $product_quantity_sub_label . "</span> ".$quantity;
			}
			else
			{
