...
class GFFormDisplay {
	...
	public static function get_form( $form_id, $display_title = true, $display_description = true, $force_display = false, $field_values = null, $ajax = false, $tabindex = 1, $new_lead = null, $customize=0 ) {
		...
		if ( empty( $confirmation_message ) ) {
			...
			if ( is_array( $form['fields'] ) ) {
				foreach ( $form['fields'] as $field ) {
					...
					if($customize==1)
					{
						$value = RGFormsModel::get_lead_field_value( $new_lead, $field );
						$display_value = GFCommon::get_lead_field_display( $field, $value, $new_lead['currency'] );
						$display_value = apply_filters( 'gform_entry_field_value', $display_value, $field, $new_lead, $form );
						$display_value = empty( $display_value ) && $display_value !== '0' ? '' : $display_value;					
						//if($field->get_input_type()=='captcha'){ continue; }
						$form_string .= self::get_field( $field, $display_value, false, $form, $field_values, 1 );					
					}
					else
					{
	...
	public static function get_field( $field, $value = '', $force_frontend_label = false, $form = null, $field_values = null, $customize=0 ) {
		$is_form_editor = GFCommon::is_form_editor();
		$is_entry_detail = GFCommon::is_entry_detail();
		$is_admin = $is_form_editor || $is_entry_detail;
		...
		//Customized line
		$field_content   = self::get_field_content( $field, $value, $force_frontend_label, $form == null ? 0 : $form['id'], $form, $customize );

		$css_class = esc_attr( $css_class );

		
		//Customized Textarea for HTML Input Area Update Every Second 
		/* HTML Content Inputs new features developed from 19 Nov 2016 */
		/* Copy Table to Div > Div inputs Convert to Span > Copy Div to Textarea for Input */
		if($input_type=='html' && stristr($field_content,'<input')){
			if($customize==1){				
				if($value=="")
				{
					$field_content = str_replace("input","span",$field_content);
					$field_container = "<li id='$field_id' class='{$css_class}' $style>{FIELD_CONTENT}</li>";
				}
				else
				{
					$value1 = html_entity_decode($value);
					$value1 = str_replace("><br />",">",$value1);
					$value1 = str_replace("<tr><br />","<tr>",$value1);
					$value1 = str_replace("<th><br />","<th>",$value1);
					$value1 = str_replace("<td><br />","<td>",$value1);
					$field_container = "<li id='$field_id' class='{$css_class}' $style>".$value1."</li>";
				}
			}
			else{
				$field_container = "<li id='$field_id' class='{$css_class}' $style>{FIELD_CONTENT}</li>";
				...
				$field_container = $field_container."<textarea style='display:none' name='input_{$id}' id='{$field_id}'></textarea><div style='display:none' name='div_input_{$id}' id='div_{$field_id}'></div>";	
									
				$html_field_id = str_replace("input","field",$field_id);			
				$field_container = $field_container.'<script type="text/javascript">function fill_html_'.$field_id.'(){ jQuery("#div_'.$field_id.'").html(jQuery("#'.$html_field_id.'").html()); jQuery("#'.$html_field_id.'").find(\'input:text\').each(function(index){ var tf = jQuery(this).val(); jQuery("#div_'.$field_id.'").find(\'input:text\').each(function(index2){ if(index==index2){ jQuery(this).val(tf); } });  }); jQuery("#div_'.$field_id.'").find(\'input\').each(function() { jQuery(this).replaceWith("<span>" + this.value + "</span>"); });  jQuery("#'.$field_id.'").html(jQuery("#div_'.$field_id.'").html());  } setInterval("fill_html_'.$field_id.'();",1000);</script>';				
			}
		}
		else
		{
		...
		//End Customized Textarea for HTML Input Area Update Every Second 
	 ...
	 * @return string
	 */
	public static function get_field_content( $field, $value = '', $force_frontend_label = false, $form_id = 0, $form = null, $customize=0 ) {
		...
		if ( $input_type == 'adminonly_hidden' ) {
			...
			$field_content = $field->get_field_content( $value, $force_frontend_label, $form, $customize );
		}
		...
		//Customized line
		$field_content = str_replace( '{FIELD}', GFCommon::get_field_input( $field, $value, 0, $form_id, $form, $customize ), $field_content );
