...
class TDWUR_Avian_Theme {
	...
	function is_active() {
		...
		if ( strtolower($current_theme->Name)=='avian' || strtolower($current_theme->Template)=='avian' ) {
			$this->active = true;
	...
	function setting_fields( $fields = array() ) {
		...
		$fields = array();
		$fields[] = array(
			'id'        => 'avian_theme_settings',
			'type'      => 'checkbox',
			'title'     => __('Avian Theme Compatibility', 'webmaster-user-role'),
	...
	function admin_menu() {
		...
		if ( empty( $webmaster_user_role_config['avian_theme_settings']['access_theme_options_panel'] ) ) remove_submenu_page( 'themes.php', 'acf-options' );
	}

}
new TDWUR_Avian_Theme();