...
class TDWUR_Ken_Theme {
	...
	function is_active() {
		...
		if ( $current_theme->Name=='ken' || $current_theme->Template=='ken' ) {
			$this->active = true;
	...
	function setting_fields( $fields = array() ) {
		...
		$fields = array();
		$fields[] = array(
			'id'        => 'ken_theme_settings',
			'type'      => 'checkbox',
			'title'     => __('Ken Theme Compatibility', 'webmaster-user-role'),
	...
	function admin_menu() {
		...
		if ( empty( $webmaster_user_role_config['ken_theme_settings']['access_theme_options_panel'] ) ) remove_menu_page( 'theme_settings' );
	}
